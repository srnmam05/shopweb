    ...
    public class BaseController : Controller
    {
        ...
        protected static decimal StarGet(int id)
        {
            ...
            using (CartsEntities db = new CartsEntities())
            {
                ...
                starcount = (from s in db.ProductCommet where s.ProductId == id select s.Stars).Count();
                if (starcount != 0)
                {
                    starsum = (from s in db.ProductCommet where s.ProductId == id select s.Stars).Sum();
        ...
        protected static List<int> StarC(int id)
        {
            ...
            using (CartsEntities db = new CartsEntities())
            {
                for (int i = 5; i >= 0; i--)
                {
                    var star = (from s in db.ProductCommet where s.ProductId == id && s.Stars == i select s.Stars).Count();
